...
public class GambleMenu {
    ...
    public void openMain(Player player) {
        ...
        // Top center info
        inv.setItem(4, buildPotItem());

        // Main games laid out symmetrically
        inv.setItem(10, item(Material.PAPER, "&6&lBlackjack",
                "&7Coming in phase 2.",
                "",
                "&8Real blackjack rules"));

        inv.setItem(11, item(Material.SUNFLOWER, "&e&lCoinflip",
                "&7Coming in phase 2.",
                "",
                "&850/50 wager game"));

        inv.setItem(13, glowingItem(Material.IRON_INGOT, "&d&lSlots",
        ...
        inv.setItem(15, item(Material.REDSTONE, "&c&lRoulette",
                "&7Coming in phase 2.",
                "",
        ...
        inv.setItem(22, item(Material.BOOK, "&b&lYour Casino Stats",
    ...
    public void openSlotsMenu(Player player) {
        ...
        // Symmetrical slot bet buttons
        for (BetTier tier : BetTier.values()) {
            ArrayList<String> lore = new ArrayList<>();
            lore.add(ColorUtil.color("&7Left-click to spin."));
            lore.add(ColorUtil.color("&7Right-click to use a free spin."));
        ...
        inv.setItem(22, item(Material.NETHER_STAR, "&6&lFree Spins",
                "&7Available: &f" + gambleService.getFreeSpins(player),
                "&7Right-click a slot bet",
                "&7to spend one free spin."));
        inv.setItem(26, item(Material.BOOK, "&b&lTip",
                "&7Small = safer",
                "&7Medium = balanced",
                "&7Big = high risk/high reward"));
    ...
    public void openSpinBase(Player player) {
        ...
        // Symmetrical frame
        inv.setItem(1, item(Material.PURPLE_STAINED_GLASS_PANE, "&5"));
        inv.setItem(4, buildPotItem());
        inv.setItem(7, item(Material.PURPLE_STAINED_GLASS_PANE, "&5"));

        inv.setItem(10, item(Material.AMETHYST_SHARD, "&d&lPAYLINE"));
        inv.setItem(16, item(Material.AMETHYST_SHARD, "&d&lPAYLINE"));

        inv.setItem(19, item(Material.PURPLE_STAINED_GLASS_PANE, "&5"));
        inv.setItem(22, item(Material.NETHER_STAR, "&6&lSpinning..."));
        inv.setItem(25, item(Material.PURPLE_STAINED_GLASS_PANE, "&5"));
